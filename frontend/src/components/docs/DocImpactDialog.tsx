...
export function DocImpactDialog({ doc, open, onOpenChange, candidateMarkdown }: Props) {
  ...
  const renderItems = (
    ...
    <div className="rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      ...
      {items.length === 0 ? (
        <p className="px-4 py-3 text-xs text-muted-foreground">{t('docImpactNone')}</p>
      ) : (
          ...
          {items.map((item) => (
            <li key={`${item.type}-${item.id}`}>
              <button
                type="button"
                onClick={() => goTo(item)}
                disabled={!linkFor(item)}
                className="flex w-full items-center gap-2 px-4 py-2 text-start text-sm hover:bg-slate-50 disabled:cursor-default disabled:hover:bg-transparent dark:hover:bg-slate-800/50"
              >
                <span className="font-mono text-xs text-muted-foreground">{item.key}</span>
                <span className="flex-1 truncate" dir="auto">{item.title}</span>
                ...
                <Badge variant="outline" className="gap-1 text-[10px]">
                  {item.reason === 'linked'
                    ? <><Link2 className="h-3 w-3" />{t('docImpactReasonLinked')}</>
                    : <><Search className="h-3 w-3" />{t('docImpactReasonSimilar')}</>}
                </Badge>
              </button>
            </li>
          ))}
  ...
  const renderRisk = (risk: DocImpactRisk, index: number) => (
    <div key={index} className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium" dir="auto">{risk.title}</span>
        <Badge variant="outline" className="ms-auto text-[10px]">{t(`docImpactArea_${risk.area}` as any)}</Badge>
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto" dir={isRTL ? 'rtl' : 'ltr'}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-violet-500" />
            {t('docImpactTitle')}
          </DialogTitle>
          <DialogDescription>{t('docImpactSubtitle')}</DialogDescription>
        </DialogHeader>
        ...
        {!loading && analysis && (
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-slate-50 px-4 py-3 text-sm dark:bg-slate-800/50">
              <p className="text-muted-foreground">{analysis.change_summary.note}</p>
              {analysis.change_summary.changed && (
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  {analysis.change_summary.headings_added.map((h) => (
                    <Badge key={`a-${h}`} className="border-0 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">+ {h}</Badge>
                  ))}
                  {analysis.change_summary.headings_removed.map((h) => (
                    <Badge key={`r-${h}`} className="border-0 bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300">− {h}</Badge>
                  ))}
                </div>
              )}
            </div>
              ...
              <div className="rounded-lg border border-violet-200 bg-violet-50/50 p-4 dark:border-violet-900/50 dark:bg-violet-950/20">
                <div className="flex items-center gap-2">
                  ...
                  {analysis.recommendation && (
                    <Badge className={`ms-auto border-0 ${recommendationTone[analysis.recommendation] || ''}`}>
                      {t(`docImpactRec_${analysis.recommendation}` as any)}
                    </Badge>
                  )}
                </div>
                {analysis.ai_summary && <p className="mt-2 text-sm" dir="auto">{analysis.ai_summary}</p>}
                {analysis.risks.length > 0 && (
                  <div className="mt-3 space-y-2">{analysis.risks.map(renderRisk)}</div>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-2 rounded-lg border border-dashed border-slate-300 px-4 py-3 text-xs text-muted-foreground dark:border-slate-700">
                <AlertTriangle className="h-4 w-4 shrink-0" />
                {analysis.ai_skipped_reason === 'no_changes' ? t('docImpactNoChanges') : t('docImpactNoAI')}
              </div>
            )}

            {/* Impacted items */}
            {renderItems(t('docImpactRequirements'), FileText, analysis.requirements)}
