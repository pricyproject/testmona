...
export function SearchableTestCaseSelect({
  id,
  value,
  onChange,
  testCases,
  ...
  const selectedTestCase = useMemo(
    () => testCases.find((testCase) => String(testCase.id) === value) || null,
    [testCases, value]
  );
  ...
  return (
    <div ref={rootRef} className={cn('relative', className)} dir={isRTL ? 'rtl' : 'ltr'}>
      <Button
        ...
        className={cn('w-full justify-between font-normal', !selectedTestCase && 'text-muted-foreground')}
      >
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full rounded-md border bg-white shadow-lg dark:border-gray-700 dark:bg-gray-900">
          ...
          <div role="listbox" className="max-h-64 overflow-y-auto py-1">
            ...
            >
              <span>{t('noTestCase')}</span>
              {value === NONE_VALUE && <Check className="h-4 w-4 text-blue-600" />}
            </button>

            {filteredTestCases.length === 0 ? (
              <div className="px-3 py-4 text-center text-sm text-gray-500">
                {normalizedQuery ? t('noTestCasesMatchSearch') : t('noTestCases')}
              </div>
            ) : (
              filteredTestCases.map((testCase) => {
                const optionValue = String(testCase.id);
          ...
          {testCases.length > MAX_VISIBLE_OPTIONS && (
            <div className="border-t px-3 py-2 text-xs text-gray-500 dark:border-gray-700">
              {t('showingTestCases', { shown: filteredTestCases.length, total: testCases.length })}
            </div>
          )}
        </div>
      )}
